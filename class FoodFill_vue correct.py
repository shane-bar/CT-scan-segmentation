...
Après lancement:
    ...
    - Sélectionner la vue de visualisation
...
class FloodFill(tk.Tk):
    def __init__(self):
        ...
        self.index = tk.Scale(self, from_=1, to=511, orient='horizontal', command = self.bind_scale_orig)
        self.index.pack()
        ...
        self.profile = tk.Radiobutton(text="sagittal", variable=self.axis, value="sagittal")
        self.profile.pack()
    ...
    def bind_scale_segm(self,event):
            ...
            if self.axis.get() == "sagittal":
                ...
                displayed_slice = self.segmented_image[:, : , slice_index]
            #print(np.shape(displayed_slice))
            # trying = np.zeros((512,302))
            # for i,k in displayed_slice:
            #     if displayed_slice[i][k] == 255:
            #         trying[i][k] == displayed_slice[i][k]
            self.displayed_slice_segm = rotate(displayed_slice, angle = -180).T
            ...
            self.rotated_segm = rotate(self.displayed_slice_segm, angle = -90)
            self.segment_canvas.config(width=self.image_segm_tk.width(), height=self.image_segm_tk.height())
            self.segment_canvas.create_image(0, 0, anchor=tk.NW, image=self.image_segm_tk)
            #self.labels = np.zeros((self.image_segm_tk.width, self.image_segm_tk.height), dtype=np.int32)
